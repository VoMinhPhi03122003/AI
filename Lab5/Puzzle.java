...
//	}
	...
	public int computeH1(Node currentState) {
		...
		for (int row = 0; row < MAX_ROW; row++) {
			...
			int tileCurrent = currentState.getTile(row, column);
			int tileGoal = currentState.getTile(row, column);
				if (currentState.getTile(row, column) != 0)
					if (currentState.getTile(row, column) != goalState.getTile(row, column))
						output++;
	...
	public int computeH2(Node currentState) {
		...
		for(int i =1 ;i<MAX_ROW*MAX_COL;i++) {
			...
			int[] currentGoal = currentState.getLocation(i);
			result += PuzzleUtils.manhattanDistance(currentPos, currentGoal);
	...
	public int computeH(Node currentState) {
		return (useH1) ? computeH1(currentState) : computeH2(currentState);
	}
	...
	public void setUseH1(boolean useH1) {
		this.useH1 = useH1;
